...
static void alsps_early_suspend(struct power_suspend *h);
static void alsps_late_resume(struct power_suspend *h);
...
static int alsps_probe(struct platform_device *pdev)
{
    ...
    register_power_suspend(&alsps_context_obj->early_drv);
